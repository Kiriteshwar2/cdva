...
from backend.app.ml.data.graph_builder import CrystalGraphDataset, GraphBuildConfig
from backend.app.ml.models import CDVAE, CDVAEModelConfig
from backend.app.ml.utils.preprocessing import ensure_directory, load_yaml_config, seed_everything, split_indices, tensor_to_python
from backend.app.ml.utils.visualization import plot_training_curves
...
def resolve_dataset_paths(config: dict[str, Any]) -> tuple[Path, Path]:
    ...
    if dataset_cfg.get("dataset_name") != "mp_20":
        raise ValueError("Offline mode requires `dataset.dataset_name: mp_20`.")
    ...
    if processed_jsonl and species_vocab:
        ...
        if processed_path.exists() and species_path.exists():
            return processed_path, species_path

    from backend.app.ml.data.mp_dataset import MPDownloadConfig, MaterialsProjectDatasetBuilder

    artifacts = MaterialsProjectDatasetBuilder(
        MPDownloadConfig(
            cache_root=Path(dataset_cfg.get("cache_root", "mp_20")),
            dataset_name="mp_20",
            max_materials=dataset_cfg.get("max_materials"),
            max_elements=dataset_cfg.get("max_elements", 20),
            max_atoms=min(int(dataset_cfg.get("max_atoms", 20)), 20),
        )
