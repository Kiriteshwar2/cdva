...
from scripts.train import load_dataset_and_splits, resolve_dataset_paths
...
def main() -> None:
    ...
    if args.checkpoint:
        ...
        cleaned_structure = clean_structure(
            structure,
            min_distance=max(float(model.config.min_distance_angstrom), 1.2),
            min_volume=max(float(model.volume_min.item()), 5.0),
        )
        if cleaned_structure is None:
            raise RuntimeError("Generated sample could not be cleaned to a physically reasonable structure.")
